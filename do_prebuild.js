...
function create_version_header() {
    const version_parts = pkg.version.split('.');
    fs.writeFileSync(
        './include/nrfdl-js-version-autogen.h',
        `// Auto-generated by do_prebuild.js\n\n\
#pragma once\n\n\
#define NRFDL_JS_VERSION_MAJOR ${version_parts[0]}\n\
#define NRFDL_JS_VERSION_MINOR ${version_parts[1]}\n\
#define NRFDL_JS_VERSION_PATCH ${version_parts[2]}`
    );
}
...
async function run_prepack(options) {
    console.log(`Pre-packing for platform: ${options.platform}.`);

    if (options.platform === 'darwin') {
        console.log('Running install_name_tool...');
        const binary_output = path.join(
            options.shared_install_dir,
            `${pkg.name.split('/')[1]}.node`
        );
        const install_name_cmd = await run_cmd(
            `install_name_tool -change @rpath/libhighlevelnrfjprog.dylib @loader_path/libhighlevelnrfjprog.dylib ${binary_output}`
        ).catch(output => output);
        console.log(install_name_cmd.stdout);
        console.log(install_name_cmd.stderr);

        if (install_name_cmd.error) {
            throw new Error(install_name_cmd.error);
        }
    }
}
