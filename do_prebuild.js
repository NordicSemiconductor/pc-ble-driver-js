...
async function decompress_local(options) {
    ...
    let version = nodeAbi.getAbi(process.versions.node, 'node');

    if (options.runtime === 'napi') {
        version = getHighestSupportedNAPIVersion();
    }
...
function getHighestSupportedNAPIVersion() {
    if (!pkg.binary || !pkg.binary.napi_versions) {
        throw new Error('Could not find napi_versions in package.json.');
    }

    const napi_version = parseInt(process.versions.napi, 10);
    let version = `${napi_version}`;

    if (!pkg.binary.napi_versions.includes(napi_version)) {
        let napi_version_found = false;
        version = '';

        pkg.binary.napi_versions.forEach((element) => {
            if (element < napi_version) {
                if (version === '' || parseInt(version, 10) < element) {
                    // Get the largest version possible.
                    version = `${element}`;
                }

                napi_version_found = true;
            }
        });

        if (!napi_version_found) {
            throw new Error(`N-API version ${napi_version} is not supported.`);
        }
    }
