...
static uECC_word_t EccPoint_compute_public_key(uECC_word_t *result,
    ...
    /* Regularize the bitcount for the var_private key so that attackers cannot use a side channel
       attack to learn the number of leading zeros. */
    carry = regularize_k(var_private, tmp1, tmp2, curve);
...
int uECC_make_key(uint8_t *public_key,
                  ...
                  uECC_Curve curve) {
#if uECC_VLI_NATIVE_LITTLE_ENDIAN
    uECC_word_t *var_private = (uECC_word_t *)private_key;
    uECC_word_t *var_public = (uECC_word_t *)public_key;
#else
    uECC_word_t var_private[uECC_MAX_WORDS];
    uECC_word_t var_public[uECC_MAX_WORDS * 2];
#endif
    uECC_word_t tries;

    for (tries = 0; tries < uECC_RNG_MAX_TRIES; ++tries) {
        if (!uECC_generate_random_int(var_private, curve->n, BITS_TO_WORDS(curve->num_n_bits))) {
            return 0;
        }

        if (EccPoint_compute_public_key(var_public, var_private, curve)) {
#if uECC_VLI_NATIVE_LITTLE_ENDIAN == 0
            uECC_vli_nativeToBytes(private_key, BITS_TO_BYTES(curve->num_n_bits), var_private);
            uECC_vli_nativeToBytes(public_key, curve->num_bytes, var_public);
            uECC_vli_nativeToBytes(
                public_key + curve->num_bytes, curve->num_bytes, var_public + curve->num_words);
#endif
            return 1;
        }
    }
...
int uECC_shared_secret(const uint8_t *public_key,
    ...
    uECC_word_t tmp[uECC_MAX_WORDS];
    uECC_word_t *p2[2] = {var_private, tmp};
...
#if uECC_VLI_NATIVE_LITTLE_ENDIAN
    bcopy((uint8_t *) var_private, private_key, num_bytes);
    bcopy((uint8_t *) var_public, public_key, num_bytes*2);
#else
    uECC_vli_bytesToNative(var_private, private_key, BITS_TO_BYTES(curve->num_n_bits));
    uECC_vli_bytesToNative(var_public, public_key, num_bytes);
    uECC_vli_bytesToNative(var_public + num_words, public_key + num_bytes, num_bytes);
#endif

    /* Regularize the bitcount for the var_private key so that attackers cannot use a side channel
       attack to learn the number of leading zeros. */
    carry = regularize_k(var_private, var_private, tmp, curve);
    ...
    EccPoint_mult(var_public, var_public, p2[!carry], initial_Z, curve->num_n_bits + 1, curve);
#if uECC_VLI_NATIVE_LITTLE_ENDIAN
    bcopy((uint8_t *) secret, (uint8_t *) var_public, num_bytes);
#else
    uECC_vli_nativeToBytes(secret, num_bytes, var_public);
#endif
    return !EccPoint_isZero(var_public, curve);
}
...
#if uECC_VLI_NATIVE_LITTLE_ENDIAN == 0
    uECC_vli_bytesToNative(var_public, public_key, curve->num_bytes);
    uECC_vli_bytesToNative(
        var_public + curve->num_words, public_key + curve->num_bytes, curve->num_bytes);
#endif
    return uECC_valid_point(var_public, curve);
}
...
int uECC_compute_public_key(const uint8_t *private_key, uint8_t *public_key, uECC_Curve curve) {
    ...
    /* Make sure the var_private key is in the range [1, n-1]. */
    if (uECC_vli_isZero(var_private, BITS_TO_WORDS(curve->num_n_bits))) {
        return 0;
    }

    if (uECC_vli_cmp(curve->n, var_private, BITS_TO_WORDS(curve->num_n_bits)) != 1) {
        return 0;
    }

    /* Compute var_public key. */
    if (!EccPoint_compute_public_key(var_public, var_private, curve)) {
        return 0;
    }
...
static int uECC_sign_with_k(const uint8_t *private_key,
    ...
    /* Prevent side channel analysis of uECC_vli_modInv() to determine
       bits of k / the var_private key by premultiplying by a random number */
...
int uECC_verify(const uint8_t *public_key,
    ...
    uECC_vli_bytesToNative(var_public, public_key, curve->num_bytes);
    uECC_vli_bytesToNative(
        var_public + num_words, public_key + curve->num_bytes, curve->num_bytes);
    ...
    /* Calculate sum = G + Q. */
    uECC_vli_set(sum, var_public, num_words);
    uECC_vli_set(sum + num_words, var_public + num_words, num_words);
